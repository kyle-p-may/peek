...
int Record::calculatePaddingSize() const {
...
std::unique_ptr<Record::Header> Record::createHeader() {
  const int valid = 1;
 ...
 */
int Record::write(std::fstream& output, std::streampos absolute) {
  if (!output.is_open()) {
...
int Record::load(std::fstream& input, std::streampos absolute) {
  if (!input.is_open()) {
...
void Record::validateRead(const Checksum_t keyChecksum_fromfile,
