...
void LogEntry::markBegin() {
  begin = true;
}

bool LogEntry::isBegin() const {
  return begin;
}

void LogEntry::markEnd() {
  end = true;
}

bool LogEntry::isEnd() const {
  return end;
}

void LogEntry::commit(std::fstream& output) {
  assert(finished);
  assert(!committed);
...
void LogEntry::finish(std::fstream& output) {
  assert(!finished);
  finished = true;    
...
bool LogEntry::hasFinished() const {
  return finished;
}

bool LogEntry::hasCommitted() const {
  return committed;
}

std::unique_ptr<LogEntry::Header> LogEntry::createHeader() {
...
void LogEntry::initFromHeader(std::unique_ptr<LogEntry::Header>& header, std::streampos pos) {
...
void LogEntry::write(std::fstream& output) {
  write(output, location);
}

int LogEntry::write(std::fstream& output, std::streampos absolute) {
  if (!output.is_open()) {
...
int LogEntry::load(std::fstream& input, std::streampos absolute) {
  if (!input.is_open()) {
